...
[estd, L, P] = kalmd(kalsys, QN, RN, 0.001);

poles_old = eig(dA'-dC'*L');

poles_new = poles_old * 0.997;
...
est_param.A = dA - L_placed*dC;
est_param.B = [dB L_placed];
...
for i = 1:length(sol.x)
    clf; hold on;
    animate(sol.x(i), sol.y(:,i), ap)

    drawnow; pause(sol.x(2)-sol.x(1)); 
end
...
problem.func.pathObj = @(t,x,u)( (t+0.1).* u.^2 );
